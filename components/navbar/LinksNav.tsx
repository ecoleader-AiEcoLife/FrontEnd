  ...
  const handleAuthAction = () => {
    if (status === 'loading') {
      <div className='flex justify-center items-center'>
        <div className='animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-white'>로딩중...</div>
      </div>;
    }
  ...
  return (
    <div className='flex items-center justify-end w-full'>
      <div className='hidden lg:flex items-center space-x-6'>
        <Link href='/about'>
          <span className='text-white font-semibold hover:text-emerald-200'>About us</span>
        </Link>
        <Link href='/board'>
          <span className='text-white font-semibold hover:text-emerald-200'>정보게시판</span>
        </Link>
        <Link href='/chatbot' onClick={handleSessionCheck}>
          <span className='text-white font-semibold hover:text-emerald-200'>재활용 챗봇</span>
        </Link>
        <Link href='/map' onClick={handleSessionCheck}>
          <span className='text-white font-semibold hover:text-emerald-200'>재활용 지도</span>
        </Link>
      </div>
      ...
      {isMenuOpen && (
        <div className='z-10 lg:hidden absolute top-full right-0 bg-green-800 p-4 mt-2 rounded-lg shadow-lg'>
          ...
          <Link href='/chatbot' className='block py-2 text-white hover:text-emerald-200' onClick={handleSessionCheck}>
            재활용 챗봇
          </Link>
          <Link href='/map' className='block py-2 text-white hover:text-emerald-200' onClick={handleSessionCheck}>
            재활용 지도
          </Link>
          {session ? (
            <button
